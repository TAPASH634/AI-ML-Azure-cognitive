...
def detect_crab(image_path, output_image_path):
    ...
    crab_count = 0  # Initialize the count of crablets
    if response.status_code == 200:
        ...
        for prediction in predictions:
            ...
            if probability > 0.90:
                crab_count += 1  # Increment the count of crablets
    ...
    return crab_count  # Return the count of crablets
...
def process_video(video_path, output_folder):
    ...
    # Create a table to store frame names and number of crablets
    table = PrettyTable()
    table.field_names = ["Frame Name", "Number of Crablets"]
    ...
    while success:
        ...
        frame_file_path = os.path.join(output_folder, f"frame_{frame_count:04d}.jpeg")
        enhanced_image.save(frame_file_path)

        # Detect crab in the enhanced image and annotate it
        detected_image_path = frame_file_path.replace(".jpeg", "_detected.jpeg")
        crab_count = detect_crab(frame_file_path, detected_image_path)

        # Add the frame name and crab count to the table
        table.add_row([os.path.basename(frame_file_path), crab_count])
